...
import smalst.nnutils.neural_renderer.neural_renderer

#from ..nnutils import geom_utils
import smalst.nnutils.geom_utils
...
class NMR(object):
    def __init__(self):
        # setup renderer
        renderer = smalst.nnutils.neural_renderer.neural_renderer.Renderer()
...
class Render(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, vertices, faces, renderer, textures=None):
        # B x N x 3
...
class NeuralRenderer(torch.nn.Module):
    ...
    def __init__(self, img_size=256, proj_type='perspective', norm_f=1., norm_z=0.,norm_f0=0.):
        ...
        if proj_type == 'perspective':
            self.proj_fn = smalst.nnutils.geom_utils.perspective_proj_withz
        else:
    ...
    def forward(self, vertices, faces, cams, textures=None):
        ...
        if textures is not None:
            ...
            return Render.apply(verts, faces, self.renderer)
